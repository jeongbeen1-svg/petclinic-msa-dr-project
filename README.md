...
- **GitOps 기반 배포 (ArgoCD)**: Git 저장소를 Single Source of Truth로 하여 EKS/AKS 양쪽 클러스터의 배포 상태를 지속적으로 동기화
- **시크릿 및 인증서 관리**: AWS Secrets Manager·ACM, Azure Key Vault로 클라우드별 민감정보를 분리 관리하고, Terraform State는 S3로 중앙화
- **이기종 DB 간 실시간 복제 (AWS DMS CDC)**: RDS(Aurora)와 Azure MySQL Flexible Server 간 CDC 방식 실시간 복제로 **RPO 1분 이내** 달성
- **통합 모니터링**: CloudWatch Container Insights(AWS)와 Prometheus(클러스터 전반)로 메트릭을 수집해 오토스케일링 정책 튜닝의 근거로 활용
...
---
## 🔗 관련 리포지토리
- [config-server (fork)](깃허브링크) — APP config-server 연동을 위해 프로파일 브랜치 구조 수정
- [k8s manifests / GitOps](깃허브링크) — EKS/AKS 배포 매니페스트, ArgoCD Application 정의
