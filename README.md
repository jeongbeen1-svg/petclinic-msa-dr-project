# ☁️ Spring PetClinic 기반 멀티클라우드 재해복구(DR) 아키텍처 구축 프로젝트
...
**🌐 Active-Standby 멀티클라우드 구조**
- **AWS (Active)**: EKS 기반 운영 트래픽 처리
- **Azure (Warm Standby)**: AKS 기반 대기 클러스터로, 평상시에도 최소 리소스로 기동하여 장애 시 빠른 전환(Failover)이 가능하도록 구성

**🔀 GitOps 기반 배포 (ArgoCD)**
Git 저장소를 Single Source of Truth로 하여 EKS/AKS 양쪽 클러스터의 애플리케이션 상태를 ArgoCD가 지속적으로 동기화하도록 구성했습니다.

**🌍 Route53 가중치/장애조치 라우팅**
- **Weighted Routing**: 평상시 트래픽 분산 정책 검증
- **Failover Routing**: AWS Active 리전 장애 감지 시 Azure Standby로 자동 전환

**🗄 이기종 DB 간 실시간 복제 (AWS DMS CDC)**
AWS RDS(Aurora)와 Azure MySQL Flexible Server 간 **CDC(Change Data Capture)** 방식의 실시간 복제를 구성하여 **RPO 1분 이내**를 달성했습니다.

**📊 통합 모니터링**
- **CloudWatch Container Insights**(`amazon-cloudwatch-observability` EKS Addon)로 AWS 측 컨테이너 메트릭 수집
- **Prometheus**로 클러스터 전반의 메트릭을 수집하여 오토스케일링 정책 튜닝의 근거 데이터로 활용
...
- GitHub Actions로 이미지 빌드 및 매니페스트 업데이트 자동화
- ArgoCD를 통해 Git 저장소 상태를 EKS/AKS 클러스터에 지속적으로 동기화
...
- **GitOps 운영 경험**: ArgoCD를 통한 선언적 배포 관리로 EKS/AKS 양쪽 클러스터의 배포 일관성 확보
...
- 상세 구축 로그 및 포트폴리오: [Notion 바로가기]([https://giddy-dryer-b71.notion.site/395fb705849380a1a2d1ec683ac19df9](https://giddy-dryer-b71.notion.site/36cfb705849380a0aa58f1173239d09f))
